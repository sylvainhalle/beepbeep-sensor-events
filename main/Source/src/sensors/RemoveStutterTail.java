...
/**
 * Removes the tail of a stuttering sequence. This processor takes as input
 * a sequence of events, and outputs the same sequence, except that the
 * last event is removed if it is identical to the one before it. What decides
 * on the equality of two events is configurable, by passing a 2:1 function
 * to the processor's constructor.
 * 
 * @author Sylvain Hallé
 */
public class RemoveStutterTail extends SynchronousProcessor
{
	/**
	...
	/**
	 * The last event that was output by this processor.
	 */
	 ...
	 */
	public RemoveStutterTail(Function comparison)
	{
		super(1, 1);
	...
	protected boolean compute(Object[] inputs, Queue<Object[]> outputs)
	{
		...
		m_comparison.evaluate(new Object[] {m_last, current}, out);
		if (Boolean.TRUE.equals(out[0]))
		{
			// The two events are considered equal; do nothing
			return true;
		}
		// The two events are different; output the current one and update
		// the reference to the last event
		outputs.add(new Object[] {current});
		m_last = current;
		return true;
	}
	...
	@Override
	public RemoveStutterTail duplicate(boolean with_state)
	{
		if (with_state)
		{
			throw new ProcessorException("Cannot duplicate this processor with state");
		}
		return new RemoveStutterTail(m_comparison);
	}
}
