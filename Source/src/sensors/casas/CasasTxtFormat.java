...
public class CasasTxtFormat implements EventFormat 
{
	...
	/*@ non_null @*/ public static final String TXT_STATE = "state"; //the message sent by the sensor
	
	/*@ non_null @*/ public static final String TXT_LOCATION = "location"; //Is the room level location of the sensor
	
	/*@ non_null @*/ public static final String TXT_SUBJECT = "subject"; // represents the more detailed location in the CASAS dataset

	/*@ non_null @*/ public static final String TXT_TIME = "time"; 
	
	/*@ non_null @*/ public static final String TXT_DATE = "date";

	/*@ non_null @*/ public static final String TXT_CONTACT = "contact"; //We do not have a contact sensors in CASAS dataset
	...
	/**
	 * In the CASAS dataset, the first two fields in an event corresponds to the date in the form "yyyy-MM-dd'
	 * and time in the form 'HH:mm:ss.SSSSSX" respectively. The two strings are extracted and
	...
	/**In the CASAS dataset, several message states are possible depending on the type of the sensor:
	 * Control4-Motion and Control4-MotionArea: report ON or OFF
	 * Control4-LightSensor: integer values ranging from 0 to 100
	 * Control4-BatteryPercent: integer ranging from 0 to 100
	 * Control4-Radio: report the message OK
	 * Control4-Door: report OPEN or CLOSE
	 * Control4-Temperature: a decimal in Celsius with 0.5 degrees Celsius accuracy
	 * Control4-Light: integer ranging from 0 to 100
	 * Control4-Button: report the messages TAP or TAP_COUNT_01 or DEPRESS or RELEASE
	 * */
	@Override
	...
	public Function sensorPlacement()
	{
		return new FunctionTree(new MergeScalars("location", "subject", "model"),
				                new FetchAttribute(TXT_LOCATION),
				                new FetchAttribute(TXT_SUBJECT),
				                new FetchAttribute(TXT_MODEL));
	}

	/**
	 * In the CASAS dataset, the placement of a sensor is uniquely determined
	 * by the combination of three attributes in an event: {@code location},
	 * {@code subject} and {@code model}.
	 */
	@Override
